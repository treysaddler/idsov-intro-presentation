...
- **Low Sovereignty:** *By them, for them* and *by them, for us* models keep decision authority external.[^carson-webinar]
- **Transitional Practice:** *By them, with us* requires negotiated governance, data-sharing terms, and enforceable community oversight.[^carson-webinar]
- **High Sovereignty:** *By us, for us* centers Indigenous institutions, Indigenous researchers, and Nation-defined priorities from design through reuse.[^carson-webinar]

::: {.notes}
Carson presents this as a practical diagnostic for partnerships.

Many projects begin in externally led models because institutions already control infrastructure and funding. But the target should always be upward movement: from consultation to co-governance, and from co-governance to Indigenous-led data systems.

This framing is useful for funders and universities because it asks a concrete question: where does authority actually sit today, and what commitments move it upward?
:::
...
- **Policy Design Matters:** Stronger compacts and formalized collaboration pathways can improve tribal-state data coordination during emergencies.[^carson-webinar]
- **Risk of Harmful Disclosure:** Publicly accessible, granular geographic case reporting can expose tribal communities to profiling and stigma if governance safeguards are weak.[^carson-webinar]
- **Operational Lesson:** Emergency plans must pre-negotiate tribal data governance, include Tribal Epidemiology Centers, and define access controls before crises begin.[^carson-webinar]

::: {.notes}
The webinar contrasts pandemic-era governance in Arizona and New Mexico to show that legal architecture, not just political rhetoric, changes outcomes.

Where collaboration is compelled by policy, tribal coordination is more reliable. Where policy is vague or optional, communities bear the consequences through delayed access, inconsistent protections, and exposed data.

For this audience, the key takeaway is preparedness: data governance cannot be improvised in a crisis. It has to be codified in advance.
:::
...
- **Identity as State Construction:** U.S. and state legal systems historically imposed identity rules (including blood quantum) to constrain Indigenous political recognition and rights.[^carson-webinar]
- **Enrollment as Governance Tool:** Tribal enrollment data is generated by Nations and tied to citizenship, but frequently treated by external institutions as administrative paperwork rather than sovereign data.[^carson-webinar]
- **Key Tension:** Self-identification and tribal affiliation can be declared by individuals, but verified citizenship status is authorized only by tribal governments.[^carson-webinar]
- **Widely Collected, Poorly Governed:** Universities, scholarship programs, health systems, and service agencies routinely request enrollment proof, but governance terms are often unclear.[^carson-webinar]
- **Asymmetry:** Tribes can issue and verify enrollment status, yet often lack practical pathways to audit how external entities store, share, or repurpose those records.[^carson-webinar]
- **Priority Actions:** Create standard notification practices, inter-institutional data-sharing agreements, and tribal-request mechanisms for retrieval, reporting, and deletion controls.[^carson-webinar]

::: {.notes}
Dr. William Carson's webinar makes an essential point that complements this deck's policy framing: the legal definition of Indigenous identity in the United States has repeatedly been shaped through state power, not Indigenous governance.

He traces how blood-quantum and related bookkeeping systems became administrative tools for narrowing recognition and access, even while tribes maintained their own systems of belonging.

The practical implication is immediate: when institutions ask students, patients, or applicants to provide enrollment verification, they are handling sovereign tribal data, not just another form upload. If that data is collected without clear tribal governance pathways, sovereignty is functionally bypassed.

Carson's enrollment-data argument is one of the most actionable parts of the webinar.

The issue is not whether services should verify eligibility. The issue is that once enrollment documentation leaves a tribal system, control often disappears.

A sovereignty-aligned model would preserve utility while restoring accountability: clear terms at collection, constraints on reuse, and enforceable rights for Nations to request records, usage reports, and corrective action.
:::
...
[^carson-webinar]: Carson, W. (2026, May 21). *Introduction to Indigenous Data Sovereignty and Tribal Enrollment Data* (webinar transcript). Alaska INBRE CBPR Core. Local transcript file in project workspace.